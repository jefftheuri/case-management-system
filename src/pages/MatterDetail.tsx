...
const MatterDetail = () => {
  ...
  // Find the matter by ID
  const matterDetail = matters.find(m => m.id === matterId);
  
  // Filter tasks for this matter
  const matterTasks = sampleTasks.filter(task => task.caseId === matterId);
    ...
    return (
      <MainLayout>
        <div className="flex flex-col items-center justify-center h-[80vh]">
          ...
          <h2 className="text-2xl font-bold">Matter Not Found</h2>
          <p className="text-muted-foreground mb-6">The matter you're looking for doesn't exist or has been removed.</p>
      ...
      <div className="space-y-6">
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h1 className="text-3xl font-bold tracking-tight">{matterDetail.title}</h1>
              <div className="flex items-center gap-2 mt-1">
                <p className="text-muted-foreground">{matterDetail.id}</p>
                <Badge 
                  variant="outline"
                  className={getStatusColor(matterDetail.status)}
                >
                  {getStatusIcon(matterDetail.status)}
                  <span className="ml-1">{matterDetail.status}</span>
                </Badge>
                <Badge className={getPriorityColor(matterDetail.priority)}>
                  {matterDetail.priority} Priority
                </Badge>
              </div>
            </div>
        ...
        <Tabs defaultValue="overview" value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="grid grid-cols-5 w-full md:w-fit">
            ...
            <TabsTrigger value="tasks">
              Tasks
              {matterTasks.filter(task => task.status !== "Completed").length > 0 && (
                <Badge className="ml-2 bg-primary text-primary-foreground">
                  {matterTasks.filter(task => task.status !== "Completed").length}
                </Badge>
              )}
            </TabsTrigger>
          ...
          <TabsContent value="overview" className="space-y-6 mt-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Card>
                <CardHeader>
                  <CardTitle className="text-xl">Matter Information</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-sm font-medium text-muted-foreground">Type</p>
                      <p>{matterDetail.type}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-muted-foreground">Filing Date</p>
                      <p>{new Date(matterDetail.filingDate).toLocaleDateString()}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-muted-foreground">Matter Number</p>
                      <p>{matterDetail.matterNumber}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-muted-foreground">Jurisdiction</p>
                      <p>{matterDetail.jurisdiction}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-muted-foreground">Court Date</p>
                      <p>{new Date(matterDetail.courtDate).toLocaleDateString()}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-muted-foreground">Due Date</p>
                      <p>{new Date(matterDetail.dueDate).toLocaleDateString()}</p>
                    </div>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-muted-foreground mb-1">Description</p>
                    <p className="text-sm">{matterDetail.description}</p>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="space-y-4">
                  <div>
                    ...
                    <Button 
                      ...
                      onClick={() => navigate(`/clients/${matterDetail.clientId}`)}
                    >
                      {matterDetail.client}
                    </Button>
                  </div>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p>{matterDetail.billableHours} hours</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-muted-foreground">Last Activity</p>
                      <p>{new Date(matterDetail.lastActivity).toLocaleDateString()}</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {new Date(matterDetail.courtDate).toLocaleDateString()}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {new Date(matterDetail.dueDate).toLocaleDateString()}
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          <TabsContent value="timeline" className="mt-6">
            <MatterTimeline matterId={matterDetail.id} />
          </TabsContent>
          
          <TabsContent value="documents" className="mt-6">
            <MatterDocuments matterId={matterDetail.id} />
          </TabsContent>
          
          <TabsContent value="tasks" className="mt-6">
            <TaskList caseId={matterDetail.id} initialTasks={matterTasks} />
          </TabsContent>
          
          <TabsContent value="notes" className="mt-6">
            <MatterNotes matterId={matterDetail.id} />
          </TabsContent>
        </Tabs>
      </div>
    </MainLayout>
